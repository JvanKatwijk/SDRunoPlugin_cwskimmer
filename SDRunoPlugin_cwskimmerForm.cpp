...
//	Form constructor with handles to parent and uno
//	controller - launches form Setup
...
// Form deconstructor
	SDRunoPlugin_cwskimmerForm::~SDRunoPlugin_cwskimmerForm () {
}

// Start Form and start Nana UI processing
void	SDRunoPlugin_cwskimmerForm::Run () {
	show ();
	nana::exec ();
}

// Create the initial plugin form
void	SDRunoPlugin_cwskimmerForm::Setup () {
	...
	hModule = GetModuleHandle(L"SDRunoPlugin_cwskimmer");
	hdc = GetDC(NULL);
	...
	*(BITMAPINFO*)(settoverPixels + sizeof(BITMAPFILEHEADER)) = bmInfo_sett_over;
	...
	title_bar_label.caption("< bold size = 6 color = 0x000000 font = \"Verdana\">cw skimmer</>");
	...
	list.append_header ("decoded text", 550);
	auto cat = list.at (0);
	for (int i = 0; i < 32; i++) {
	   cat.append ({std::to_string (i), ""});
	}
...
void	SDRunoPlugin_cwskimmerForm::setText	(int index,
	                                         int freq,
	                                         int wpm,
	                                         const std::string &s) {
	list. at (nana::listbox::index_pair (0, index)).
	                                        text (1, std::to_string (freq));
	list. at (nana::listbox::index_pair (0, index)).
	                                        text (2, std::to_string (wpm));
	list. at (nana::listbox::index_pair (0, index)). text (3, s);
}
...
void	SDRunoPlugin_cwskimmerForm::reset_width	(int n) {
